...
begin
  ...
  begin
    ...
    if sanitizeurl(fbasepath,val,localpath,localname,fn) then
    begin
      compileLocalLinks := compileLocalLinks + 1;
      found := true;
    end;
  end;
...
procedure checkattributesIMG(node:TDomNode;const localname: string; filelist :TStringList);
begin
  compileGraphics := compileGraphics + 1;
end;
            ...
            begin
             ...
             if s='IMG' then
               begin
                  //printattributes(chld,'');
                  //checkattributes(chld,'SRC',localname,filelist);
                checkattributesIMG(chld,localname,filelist);
               end;
             if s='A' then
               begin
                  //printattributes(chld,'');
